...
# build
CC=arm-none-eabi-gcc CXX=arm-linux-gnu-g++ CC_host="gcc -m32" CXX_host="g++ -m32" ./configure --prefix=../install --dest-cpu=arm --cross-compiling --dest-os=linux --with-arm-float-abi=hard --with-arm-fpu=neon
make -j4
make install
